...
export function startClaudeCodeScreenIntermediateMonitor(options: {
  ...
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      if (shouldPublishPrefixIntermediate(text, lastText)) {
        lastText = text;
...
export function shouldPublishPrefixIntermediate(text: string | null, previousText: string | null): text is string {
  if (!text || !text.trim()) {
    return false;
  }
  ...
  return previousText === null || text.startsWith(previousText);
}
