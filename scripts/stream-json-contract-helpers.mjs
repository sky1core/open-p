...
export function previewResultCompatibilityReport(texts, resultText) {
  const resultNormalized = normalizeComparableText(resultText);
  const resultRenderedNormalized = normalizeComparableText(renderMarkdownForTerminalComparison(resultText));
  ...
  const samples = texts.map((text) => {
    ...
    const resultStartsWithRawPrefix = checkedPrefixLength > 0 &&
      resultNormalized.startsWith(checkedPrefix);
    const resultRenderedStartsWithPrefix = checkedPrefixLength > 0 &&
      resultRenderedNormalized.startsWith(checkedPrefix);
    return {
      textLength: text.length,
      normalizedLength: normalized.length,
      checkedPrefixLength,
      resultStartsWithCheckedPrefix: resultStartsWithRawPrefix || resultRenderedStartsWithPrefix,
      resultStartsWithRawPrefix,
      resultRenderedStartsWithPrefix,
    };
  });
  return {
    resultLength: typeof resultText === 'string' ? resultText.length : null,
    resultNormalizedLength: resultNormalized.length,
    resultRenderedNormalizedLength: resultRenderedNormalized.length,
    requiredPrefixLength,
    samples,
    compatible: samples.some((sample) => sample.resultStartsWithCheckedPrefix) &&
      samples.some((sample) => sample.checkedPrefixLength >= requiredPrefixLength) &&
      samples.every((sample) => sample.resultStartsWithCheckedPrefix),
  };
}
