...
const GOLDEN = join(import.meta.dirname, 'fixture-kiro-golden.jsonl');
const COMPANION = join(import.meta.dirname, 'fixture-kiro-golden.json');
...
test('appendKiroSessionHistory rejects an aborted signal before the write and leaves the log untouched', async () => {
  const home = await mkdtemp(join(tmpdir(), 'openp-kiro-home-'));
  await withHome(home, async () => {
    const sessionId = randomUUID();
    const logPath = resolveKiroSessionLogPath(sessionId, { HOME: home })!;
