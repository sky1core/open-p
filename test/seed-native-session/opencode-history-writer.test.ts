...
const GOLDEN = join(import.meta.dirname, 'fixture-opencode-golden-export.json');
const COMPACTED = join(import.meta.dirname, 'compacted-session-export.json');
const NOW_MS = Date.UTC(2026, 6, 14, 12, 0, 0);
const TURNS: readonly SeedWriteTurn[] = [
  { logicalId: 'turn-1', userText: 'codename FIXTURESECRET', assistantText: 'noted one', contentDigest: 'digest-1', sourceNativeIds: null },
  { logicalId: 'turn-2', userText: 'fixture month marker', assistantText: 'noted two', contentDigest: 'digest-2', sourceNativeIds: null },
];
...
test('OpenCode writer never clones synthetic or ignored text into portable seed turns', async () => {
  ...
  user.parts.push({
    id: 'prt_000000000a01AAAAAAAAAAAAAA',
  ...
  assistant.parts.push({
    id: 'prt_000000000a02AAAAAAAAAAAAAA',
    sessionID: before.info.id,
...
test('OpenCode writer never clones a tool-loop intermediate assistant as its terminal template', async () => {
  ...
  user.info.id = 'msg_000000000a05AAAAAAAAAAAAAA';
  user.info.time = { created: NOW_MS - 4 };
  user.parts = [{
    id: 'prt_000000000a06AAAAAAAAAAAAAA',
    sessionID: before.info.id,
    messageID: user.info.id,
    type: 'text',
  ...
  intermediate.info.id = 'msg_000000000a07AAAAAAAAAAAAAA';
  ...
  intermediate.parts = [
    {
      id: 'prt_000000000a08AAAAAAAAAAAAAA',
      sessionID: before.info.id,
    ...
    {
      id: 'prt_000000000a09AAAAAAAAAAAAAA',
      sessionID: before.info.id,
      messageID: intermediate.info.id,
      type: 'tool',
  ...
  for (const [variant, tool] of [
    ...
    assistant.parts.push({
      id: variant === 'provider-executed-template'
        ? 'prt_000000000a0cAAAAAAAAAAAAAA'
        : 'prt_000000000a0dAAAAAAAAAAAAAA',
      sessionID: before.info.id,
      messageID: assistant.info.id,
      type: 'tool',
      ...tool,
...
test('prepareOpenCodeHistoryAppend rejects a trailing incomplete target before import', async () => {
  ...
  exportDoc.messages.push({
    info: {
      ...
      id: 'msg_000000000a03AAAAAAAAAAAAAA',
      role: 'user',
    ...
    parts: [{
      ...
      id: 'prt_000000000a04AAAAAAAAAAAAAA',
      messageID: 'msg_000000000a03AAAAAAAAAAAAAA',
      sessionID: exportDoc.info.id,
      type: 'text',
      text: 'unfinished',
    }],
