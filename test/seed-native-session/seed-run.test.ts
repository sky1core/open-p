  ...
  await withStateRoot(async (projectRoot, stateRoot) => {
    ...
    await writeFile(irPath, JSON.stringify({
      ...
      turns: [{
        ...
        user: { text: 'codename FIXTURESECRET' },
    ...
    assert.equal(stored.includes('codename FIXTURESECRET'), false);
...
test('operation create records creating before bootstrap and target-created before native mutation', async () => {
  await withStateRoot(async (projectRoot, stateRoot) => {
    const sensitiveExternalId = 'raw external id FIXTURESECRET';
    const sensitiveUser = 'codename FIXTURESECRET user text';
    const sensitiveAssistant = 'codename FIXTURESECRET assistant text';
