...
const SEED_ASSET_DIR = import.meta.dirname;
...
// Imports the synthetic golden export under a custom session id into openp's isolated opencode store
...
async function importOpenCodeSession(
  ...
  const fixture = JSON.parse(await readFile(join(SEED_ASSET_DIR, 'fixture-opencode-golden-export.json'), 'utf8'));
...
async function writeExternalIr(dir: string): Promise<string> {
  ...
  await writeFile(path, JSON.stringify({
    schemaVersion: 1,
    turns: [{ id: 'one', user: { text: 'codename FIXTURESECRET' }, assistant: { text: 'noted' } }],
  }));
  return path;
}
...
async function installKiroFixture(home: string, sessionId: string): Promise<{ readonly logPath: string; readonly companionPath: string }> {
  ...
  const logText = await readFile(join(SEED_ASSET_DIR, 'fixture-kiro-golden.jsonl'), 'utf8');
  const companion = JSON.parse(await readFile(join(SEED_ASSET_DIR, 'fixture-kiro-golden.json'), 'utf8'));
...
async function installCodexFixture(
  home: string,
  sessionId: string,
  fixture = 'fixture-codex-golden.jsonl',
): Promise<string> {
  const logPath = join(home, 'sessions', '2026', '07', '14', `rollout-2026-07-14T00-00-00-${sessionId}.jsonl`);
  await mkdir(dirname(logPath), { recursive: true });
  const entries = (await readFile(join(SEED_ASSET_DIR, fixture), 'utf8'))
...
async function installClaudeFixture(
  ...
  sessionId: string,
  cwd: string,
  fixture = 'fixture-claude-golden.jsonl',
): Promise<string> {
...
test('seed operation create replay returns identical CLI stdout without launching a second target', async () => {
  ...
  const env = await withFakeCommandEnv(
    'codex',
    join(repoRoot, 'test', 'seed-native-session', 'fake-codex-seed-bootstrap.mjs'),
    {
      XDG_STATE_HOME: stateRoot,
...
test('seed operation recovers a SIGKILL-stale creating lock as indeterminate without another target launch', {
  ...
  const env = await withFakeCommandEnv(
    'codex',
    join(repoRoot, 'test', 'seed-native-session', 'fake-codex-seed-hang.mjs'),
    {
      XDG_STATE_HOME: stateRoot,
      OPENP_FAKE_CODEX_ARGS_LOG: argsLog,
...
test('configured Claude source seeds a configured Codex target through instance-owned roots', async () => {
  ...
  const targetPath = await installCodexFixture(codexHomeDir, targetSessionId, 'fixture-codex-bootstrap.jsonl');
...
test('configured Codex source seeds a configured Claude target through instance-owned roots', async () => {
  ...
  const targetPath = await installClaudeFixture(
    ...
    targetSessionId,
    projectRoot,
    'fixture-claude-bootstrap.jsonl',
  );
  const sourceBytes = await readFile(sourcePath);
