...
const OPENCODE_ASSET_DIR = import.meta.dirname;
...
test('Claude reader extracts completed caller/final assistant pairs and drops trailing incomplete users', async () => {
  const logText = await readFile(join(SESSION_ASSET_DIR, 'fixture-claude-golden.jsonl'), 'utf8');
  ...
  assert.equal(turns[0]!.assistantText, 'FIXTURE_TOKEN_A');
  assert.equal(turns[1]!.assistantText, 'FIXTURE_TOKEN_B');
...
test('Claude reader scopes session-log cwd to the workspace and its descendants', async () => {
  ...
  assert.equal(turns.length, 2);
  assert.deepEqual(turns.map((turn) => turn.assistantText), ['FIXTURE_TOKEN_A', 'FIXTURE_TOKEN_B']);
...
test('Claude reader recovers completed turns across a compact boundary and never emits the summary', async () => {
  ...
  assert.deepEqual(
    ...
    ['FIXTURE_TOKEN_A', 'FIXTURE_TOKEN_B', 'THIRD-ANSWER', 'FOURTH-ANSWER'],
  );
...
test('Claude reader drops a segment-trailing incomplete turn at a compact boundary', async () => {
  ...
  const turns = extractClaudeNativeTurns(`${logText}\n${compacted}\n`);
  assert.equal(turns.length, 3);
  assert.deepEqual(turns.map((turn) => turn.assistantText), ['FIXTURE_TOKEN_A', 'FIXTURE_TOKEN_B', 'THIRD-ANSWER']);
...
test('Claude reader drops an interrupted turn and continues from the next caller user', async () => {
  ...
  const turns = extractClaudeNativeTurns(`${logText}\n${interrupted}\n`);
  assert.equal(turns.length, 3);
  assert.deepEqual(turns.map((turn) => turn.assistantText), ['FIXTURE_TOKEN_A', 'FIXTURE_TOKEN_B', 'SECOND-ANSWER']);
...
test('Claude reader emits active records in parent-lineage order when tool records are appended out of order', () => {
  const records = [
    { type: 'system', subtype: 'init', uuid: 'root-status', parentUuid: null },
    {
      type: 'user', uuid: 'caller-user', parentUuid: 'root-status',
      message: { role: 'user', content: 'use the tool' },
    },
    {
      type: 'assistant', uuid: 'final-assistant', parentUuid: 'tool-result',
      message: { id: 'final-message', role: 'assistant', content: [{ type: 'text', text: 'FINAL-ANSWER' }] },
    },
    {
      type: 'user', uuid: 'tool-result', parentUuid: 'tool-use-assistant',
      message: {
        role: 'user',
        content: [{ type: 'tool_result', tool_use_id: 'tool-use-1', content: 'TOOL-RESULT' }],
      },
    },
    {
      type: 'assistant', uuid: 'tool-use-assistant', parentUuid: 'caller-user',
      message: {
        id: 'tool-use-message',
        role: 'assistant',
        content: [
          { type: 'text', text: 'TOOL-USE-PREAMBLE' },
          { type: 'tool_use', id: 'tool-use-1', name: 'lookup', input: { query: 'synthetic' } },
        ],
      },
    },
    {
      type: 'system', subtype: 'turn_duration', uuid: 'completion',
      parentUuid: 'final-assistant', durationMs: 1,
    },
  ];

  const turns = extractClaudeNativeTurns(records.map((entry) => JSON.stringify(entry)).join('\n'));
  assert.equal(turns.length, 1);
  assert.equal(turns[0]!.assistantText, 'TOOL-USE-PREAMBLE\n\nFINAL-ANSWER');
  assert.deepEqual(turns[0]!.nativeIds, {
    userId: 'caller-user',
    assistantIds: ['tool-use-message', 'final-message'],
    completionId: 'completion',
  });
});

test('Claude reader accepts a segment whose lineage root is a meta record behind uuid-less head records', () => {
  // Claude Code opens a log with head records that carry no uuid at all, so the segment's first uuid
  // — and therefore its lineage root — can be a meta record that no portable turn ever uses. Anchor
  // the root on that first uuid rather than on what the root record looks like, or these sessions
  // stop reading.
  const records = [
    { type: 'mode', mode: 'fixture-mode' },
    { type: 'file-history-snapshot', snapshotId: 'fixture-snapshot' },
    { type: 'user', uuid: 'meta-root', parentUuid: null, isMeta: true, message: { role: 'user', content: 'fixture meta notice' } },
    { type: 'user', uuid: 'caller-user', parentUuid: 'meta-root', message: { role: 'user', content: 'fixture prompt' } },
    {
      type: 'assistant', uuid: 'answer', parentUuid: 'caller-user',
      message: { id: 'answer-message', role: 'assistant', content: [{ type: 'text', text: 'FIXTURE-ANSWER' }] },
    },
    { type: 'system', subtype: 'turn_duration', uuid: 'completion', parentUuid: 'answer', durationMs: 1 },
  ];

  const turns = extractClaudeNativeTurns(records.map((entry) => JSON.stringify(entry)).join('\n'));
  assert.equal(turns.length, 1);
  assert.equal(turns[0]!.userText, 'fixture prompt');
  assert.equal(turns[0]!.assistantText, 'FIXTURE-ANSWER');
});
...
test('Claude reader rejects malformed parent lineage without truncating or looping', async () => {
  ...
  rejects(() => extractClaudeNativeTurns([
    JSON.stringify(root),
    JSON.stringify(user),
    JSON.stringify({ ...assistant, parentUuid: null }),
    JSON.stringify(completion),
  ].join('\n')));
  ...
  // Case A: the assistant appears before its parent user in file order, but the parentUuid chain is
  // intact. Lineage now defines active order, so this remains a valid completed turn.
  assert.equal(extractClaudeNativeTurns([
    JSON.stringify(root),
    JSON.stringify(assistant),
    JSON.stringify(user),
    JSON.stringify(completion),
  ...
  // Case B: the chain terminates at a root that is not the segment's first uuid. Lineage order
  // decides how active records are emitted, but the segment root itself is still anchored to the
  // first uuid in the file: a null-parent record appearing after its own child means the records
  // before it lost their link to the active chain, which is severed lineage rather than the local
  // writer reordering this reader tolerates.
  rejects(() => extractClaudeNativeTurns([
    JSON.stringify(user),
    JSON.stringify(root),
    JSON.stringify(assistant),
  ...
  rejects(() => extractClaudeNativeTurns([
    ...
    JSON.stringify({
      ...
      uuid: 'tool-result-1',
      parentUuid: null,
      message: { role: 'user', content: [{ type: 'tool_result', tool_use_id: 'tool-use-1', content: 'orphaned' }] },
    }),
    JSON.stringify({ ...assistant, parentUuid: 'tool-result-1' }),
    JSON.stringify(completion),
  ].join('\n')));
...
test('Claude reader handles deep active lineages without recursive stack overflow', () => {
  ...
  // A fully reversed file walks every one of the 15,001 links iteratively before reaching the root,
  // which is what this case exercises. It then fails closed because that root is not the segment's
  // first uuid: whole-file reversal is severed lineage, not the local reordering a fast tool result
  // produces.
...
test('Codex reader extracts task-complete paired turns and ignores developer/reasoning mirrors', async () => {
  const logText = await readFile(join(SESSION_ASSET_DIR, 'fixture-codex-golden.jsonl'), 'utf8');
  const trailingUser = JSON.stringify({
    type: 'response_item',
    payload: {
      type: 'message',
  ...
  assert.equal(recoveredTrailing[1]!.assistantText, 'FIXTURE_TOKEN_B');
...
test('Codex reader closes a window missing task_complete at the next task_started', async () => {
  ...
  assert.equal(recovered[0]!.assistantText, 'FIXTURE_TOKEN_A');
  assert.equal(recovered[0]!.nativeIds.completionId, turns[0]!.nativeIds.completionId);
  assert.equal(recovered[1]!.assistantText, 'FIXTURE_TOKEN_B');
...
const CODEX_OLDFORMAT_SESSION_META = {
  type: 'session_meta',
  payload: { id: '019e0000-0000-7000-8000-000000000001', cli_version: '9.9.1', originator: 'codex_exec', source: 'exec' },
};
...
test('Codex reader ignores passthrough-bound injected user records without an adjacent mirror', () => {
  ...
  const sessionMeta = {
    type: 'session_meta',
    payload: { id: '019e0000-0000-7000-8000-000000000002', cli_version: '9.9.2', originator: 'codex_exec', source: 'exec' },
  };
...
test('Kiro reader requires JSONL plus companion completion metadata', async () => {
  const logText = await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.jsonl'), 'utf8');
  const companionText = validKiroCompanion(logText, await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.json'), 'utf8'));
...
test('Kiro reader rejects non-object companion turn metadata', async () => {
  ...
  const companion = JSON.parse(validKiroCompanion(
    logText,
    await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.json'), 'utf8'),
  ));
...
test('Kiro reader rejects companion drift and mid-history unproven prompts', async () => {
  const logText = await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.jsonl'), 'utf8');
  const rawCompanionText = await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.json'), 'utf8');
...
test('Kiro reader skips compaction records and allows the unproven trailing turn', async () => {
  ...
  // metadata yet.
  const logText = await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.jsonl'), 'utf8');
  const companion = JSON.parse(await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.json'), 'utf8'));
  ...
  const compactedLog = [
    ...
    JSON.stringify({
      ...
      data: {
        ...
        content: [{ kind: 'text', data: 'fixture kiro prompt gamma' }],
        meta: { timestamp: 1784030003 },
      },
    }),
    JSON.stringify({
      version: 'v1',
      ...
      data: {
        ...
        content: [{ kind: 'text', data: 'FIXTURE_TOKEN_C' }],
      },
    }),
  ...
  assert.equal(turns[0]!.assistantText, 'FIXTURE_TOKEN_A');
  assert.equal(turns[1]!.assistantText, 'FIXTURE_TOKEN_B');
  assert.equal(turns[2]!.assistantText, 'FIXTURE_TOKEN_C');
...
test('OpenCode reader extracts completed exported pairs and drops trailing incomplete user messages', async () => {
  const exportDoc = JSON.parse(await readFile(join(SESSION_ASSET_DIR, 'fixture-opencode-golden-export.json'), 'utf8'));
  const before = JSON.stringify(exportDoc);
  exportDoc.messages.push({
    info: { id: 'msg_00000000090dAAAAAAAAAAAAAA', role: 'user' },
    parts: [{ id: 'prt_00000000090eAAAAAAAAAAAAAA', type: 'text', text: 'unfinished' }],
  });
  const withTrailingUser = JSON.stringify(exportDoc);
  const turns = extractOpenCodeNativeTurns(withTrailingUser);
  assert.equal(turns.length, 1);
  assert.equal(turns[0]!.assistantText.trim(), 'FIXTURE_OPENCODE_OK');
...
test('native state digests cover every backend-owned component but ignore OpenCode object key order', async () => {
  const kiroLog = await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.jsonl'), 'utf8');
  const kiroCompanion = await readFile(join(SESSION_ASSET_DIR, 'fixture-kiro-golden.json'), 'utf8');
  assert.notEqual(
...
test('OpenCode reader skips completed compaction marker pairs and recovers surrounding turns', async () => {
  const exportDoc = JSON.parse(await readFile(join(OPENCODE_ASSET_DIR, 'compacted-session-export.json'), 'utf8'));
  ...
  const corruptions: readonly (readonly [string, (doc: any) => void])[] = [
    ...
    ['compaction user has text part', (doc) => {
      doc.messages[4].parts.push({
        id: 'prt_000000000911AAAAAAAAAAAAAA',
    ...
    ['summary assistant has compaction part', (doc) => {
      doc.messages[5].parts.push({
        id: 'prt_000000000912AAAAAAAAAAAAAA',
        sessionID: doc.info.id,
...
test('OpenCode reader rejects pending revert and malformed OpenCode source sequences', async () => {
  ...
  compacted.messages[0].parts.push({
    id: 'prt_000000000901AAAAAAAAAAAAAA',
  ...
  nonAdjacent.messages.splice(1, 0, {
    info: { id: 'msg_000000000902AAAAAAAAAAAAAA', role: 'system' },
    parts: [],
  });
  ...
  consecutiveUsers.messages.splice(1, 0, {
    info: { id: 'msg_000000000903AAAAAAAAAAAAAA', role: 'user' },
    parts: [{ id: 'prt_000000000904AAAAAAAAAAAAAA', type: 'text', text: 'second pending user' }],
  });
  ...
  middleIncomplete.messages.push(
    {
      info: { id: 'msg_000000000905AAAAAAAAAAAAAA', role: 'user' },
      parts: [{ id: 'prt_000000000906AAAAAAAAAAAAAA', type: 'text', text: 'later user' }],
    },
    {
      info: {
        id: 'msg_000000000907AAAAAAAAAAAAAA', role: 'assistant',
        parentID: 'msg_000000000905AAAAAAAAAAAAAA', finish: 'stop',
        time: { created: 1700000000001, completed: 1700000000002 },
      },
      parts: [{ id: 'prt_000000000908AAAAAAAAAAAAAA', type: 'text', text: 'later answer' }],
    },
  );
...
test('OpenCode reader binds every assistant subturn to its user and keeps the terminal completion id', async () => {
  ...
  intermediate.info.id = 'msg_000000000902AAAAAAAAAAAAAA';
  ...
  intermediate.parts = [{
    id: 'prt_000000000903AAAAAAAAAAAAAA',
  ...
  toolContinuedSibling.messages[1].parts.push({
    id: 'prt_000000000904AAAAAAAAAAAAAA',
  ...
  for (const corrupt of [
    ...
    (doc: any) => { doc.messages[1].info.parentID = 'msg_000000000904AAAAAAAAAAAAAA'; },
  ]) {
    const candidate = structuredClone(exportDoc);
    corrupt(candidate);
    rejects(() => extractOpenCodeNativeTurns(JSON.stringify(candidate), candidate.info.id));
...
test('OpenCode reader excludes explicit provider-error turns and rejects false completion evidence', async () => {
  ...
  nextUser.info.id = 'msg_00000000090fAAAAAAAAAAAAAA';
  nextUser.parts[0].id = 'prt_000000000910AAAAAAAAAAAAAA';
  ...
  pendingTool.messages[1].parts.push({
    id: 'prt_00000000090aAAAAAAAAAAAAAA',
...
test('OpenCode reader excludes synthetic internal user text from caller IR', async () => {
  ...
  mixed.messages[0].parts.push({
    id: 'prt_000000000909AAAAAAAAAAAAAA',
