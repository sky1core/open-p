...
import { readinessTimeoutMs, waitForClaudeCodeInputReady } from '../src/backends/claude-code/interactive.js';
import { PersistentClaudeCodeProcess, startPersistentClaudeCodeProcess } from '../src/backends/claude-code/persistent-process.js';
...
test('persistent background watcher does not route active assistant text as background text when notification comes first', async () => {
  ...
  try {
    ...
    assert.equal(secondTurnBackgroundTexts.some((text) => text.includes('active final')), false);
  } finally {
    await process.shutdown();
  }
});
...
test('persistent turn starts JSONL parsing at the prompt submission boundary', async () => {
  ...
  try {
    ...
    assert.equal(result.text, 'active final');
    assert.deepEqual(intermediate, ['active final']);
  } finally {
    await process.shutdown();
  }
});
