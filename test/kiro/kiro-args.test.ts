...
test('buildKiroAcpArgs rejects a native permission mode instead of ignoring it', () => {
  // Kiro has no permission-mode selector: --trust-all-tools is one boolean and --trust-tools names
  // individual tools. Accepting the option and dropping it would run the turn on whatever Kiro
  // defaults to while the caller believes it was restricted.
  assert.throws(
    () => buildKiroAcpArgs({ nativeExecutionMode: 'fixture-mode-alpha' }),
