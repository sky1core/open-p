...
test('extractKiroTurnResult preserves synthetic Kiro long-answer session-log fixture', () => {
  const result = extractKiroTurnResult(readKiroFixture('fixture-session-log-long-answer-no-tool.jsonl'));
  const publicOutput = openPOutputForKiroResult(result);

  assert.equal(result.text?.length, 942);
...
test('extractKiroTurnResult preserves synthetic Kiro tool-use session-log fixture artifacts', () => {
  const result = extractKiroTurnResult(readKiroFixture('fixture-session-log-tool-use-file.jsonl'));
  const blocks = firstBlocks(result.assistantEvents);
  const publicOutput = openPOutputForKiroResult(result);

  assert.equal(result.text?.includes('sum=15'), true);
  assert.equal(result.text?.length, 215);
...
test('extractKiroTurnResult preserves synthetic Kiro denied permission tool-result fixture artifacts', () => {
  const result = extractKiroTurnResult(readKiroFixture('fixture-session-log-permission-tool-use.jsonl'));
  const blocks = firstBlocks(result.assistantEvents);

  assert.equal(result.text?.includes('쓰기 권한이 거부되어'), true);
  assert.equal(result.text?.length, 186);
  assert.deepEqual(result.toolsUsed, ['read', 'write']);
  assert.deepEqual(blocks.map((block) => block.type), ['tool_use', 'tool_result', 'tool_use', 'tool_result']);
  assert.equal(blocks[1].tool_use_id, blocks[0].id);
  assert.equal(blocks[3].tool_use_id, blocks[2].id);
