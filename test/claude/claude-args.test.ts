...
test('passes a native plan mode through even when the tool policy is read-only', () => {
  const args = buildClaudeCodeArgs({
    ...OPTIONS,
    permissionMode: null,
    nativePermissionMode: 'plan',
...
test('passes a native plan mode through whatever the combined tool policy is', () => {
  const args = buildClaudeCodeArgs({
    ...OPTIONS,
    permissionMode: null,
    nativePermissionMode: 'plan',
...
test('persistent Claude Code args pass a native plan mode through unchanged', () => {
  const args = buildPersistentClaudeCodeArgs({
    sessionId: '11111111-1111-4111-8111-111111111111',
    resume: false,
    cwd: '/tmp/workspace',
