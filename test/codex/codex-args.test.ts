...
test('sends a native permission mode to the Codex sandbox selector on a first turn', () => {
  // The value belongs to Codex; open-p keeps no list of what it accepts. Approval is pinned because
  // `codex exec` has no flag for it and a policy that asks refuses the call instead of prompting.
  const args = buildFirstTurnArgs({ ...BASE_OPTIONS, nativeExecutionMode: 'fixture-mode-alpha' });
  const at = args.indexOf('--sandbox');
  assert.notEqual(at, -1, 'expected a --sandbox selector');
  assert.equal(args[at + 1], 'fixture-mode-alpha');
  assert.equal(args[args.indexOf('--ask-for-approval') + 1], 'never');
});

test('sends a native permission mode as a resume config override', () => {
  // `codex exec resume` has no --sandbox flag, so the same selection travels as config.
  const args = buildResumeTurnArgs('session-uuid', { ...BASE_OPTIONS, nativeExecutionMode: 'fixture-mode-alpha' });
  const joined = args.join(' ');
  assert.match(joined, /sandbox_mode="fixture-mode-alpha"/);
