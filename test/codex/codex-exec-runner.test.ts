...
test('runCodexExec force kills a process that ignores graceful interruption on timeout', async () => {
  ...
  const result = await runCodexExec({
    ...
    cwd: process.cwd(),
    env: { ...process.env, OPENP_FAKE_CODEX_SIGNAL_LOG: signalLog },
    // The timeout must not fire while the fixture is still booting: a SIGINT before the fixture
    // installed its handlers kills it outright and the test then observes the wrong signal and an
    // empty log. Loaded-machine boot has been observed to pass one second.
    timeoutMs: 5000,
...
test('runCodexExec aborts running process via signal', async () => {
  ...
  // The shell fixture must have executed its trap lines before the SIGINT lands, or it dies by
  // the default action without writing the signal log asserted below.
...
test('runCodexExec keeps user abort classified as abort when timeout is also configured', async () => {
  ...
  const result = await runCodexExec({
    ...
    cwd: process.cwd(),
    env: { ...process.env, OPENP_FAKE_CODEX_SIGNAL_LOG: signalLog },
    // Large enough that the abort always precedes the timeout: the abort is scheduled relative to
    // the fixture's ready line, and a loaded machine can push boot past a small timeout, flipping
    // the classification under test to timeout.
    timeoutMs: 30000,
    signal: ac.signal,
    interruptGraceMs: 250,
    terminateGraceMs: 2000,
...
test('runCodexExec keeps timeout classified as timeout when abort arrives after timeout', async () => {
  ...
  const result = await runCodexExec({
    ...
    cwd: process.cwd(),
    env: { ...process.env, OPENP_FAKE_CODEX_SIGNAL_LOG: signalLog },
    // Boot margin, as above: the timeout's SIGINT must land after the fixture installed its
    // handlers. The abort delay counts from the ready line, which is at or after spawn, so
    // ready + 6000 is always past spawn + 5000 and the timeout always classifies first.
    timeoutMs: 5000,
    signal: ac.signal,
    interruptGraceMs: 10000,
    terminateGraceMs: 2000,
    onStdoutLine: scheduleAbortAfterReady(ac, 6000),
  });
...
test('runCodexExec repeated abort signal escalates before interrupt grace expires', async () => {
  ...
  // Escalate only after the previous signal shows up in the log, so a loaded machine can never
  // SIGKILL the fixture before its SIGINT/SIGTERM handlers had the event-loop time to log.
  await ready;
  ac.abort();
  assert.deepEqual(await readSignalLog(signalLog), ['SIGINT']);
  force.abort();
...
test('runCodexExec includes lines arriving between child exit and grace settle in the result', async () => {
  // The parent fixture exits once its grandchild finished booting; the
  // grandchild inherits the stdout pipe, writes one JSONL line as soon as it
  // observes the parent's death, and holds the pipe open until released — so
  // the line lands inside the post-exit grace window and the runner settles
  // through the grace timer instead of the close event.
...
test('runCodexExec does not invoke onStdoutLine after the result settles', async () => {
  ...
  // The grandchild writes one line as soon as the parent's exit reparents
  // it, then waits for the late release file before writing a second line —
  // which therefore arrives only after the grace settle resolved the promise.
  const result = await runCodexExec({
    bin: join(FIXTURES, 'fake-codex-orphan-stdout.mjs'),
    args: [],
    cwd: process.cwd(),
...
// waitForFile only proves the file exists; the signal the assertion is about may not be appended
// yet, and a second signal lands later still. Read until the expected count is there.
