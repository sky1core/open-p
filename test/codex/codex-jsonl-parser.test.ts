...
test('parseCodexOutput preserves synthetic Codex long-answer stdout fixture', () => {
  const parsed = parseCodexOutput(readCodexFixture('fixture-stdout-long-answer-no-tool.jsonl'), null);

  assert.equal(parsed.content?.length, 1124);
...
test('parseCodexOutput preserves synthetic Codex tool-use stdout fixture output kinds', () => {
  const stdout = readCodexFixture('fixture-stdout-tool-use-file.jsonl');
  ...
  assert.equal(parsed.content?.includes('sum=15'), true);
  assert.equal(parsed.content?.length, 160);
...
test('processCodexStdoutLine streams synthetic Codex tool-use stdout fixture as stdout-local cumulative snapshots', () => {
  const streaming = collectCodexStdoutStreaming(readCodexFixture('fixture-stdout-tool-use-file.jsonl'));

  assert.equal(streaming.answerSnapshots.length, 4);
  assert.equal(streaming.answerSnapshots[0]?.startsWith('fixture 계획:'), true);
  ...
  assert.equal(streaming.answerSnapshots[3]?.length, 383);
...
test('parseCodexOutput preserves synthetic Codex structured-output stdout fixture as JSON text', () => {
  const parsed = parseCodexOutput(readCodexFixture('fixture-stdout-structured-output.jsonl'), null);
  const structured = JSON.parse(parsed.content ?? 'null');

  assert.equal(parsed.content?.length, 290);
  assert.equal(parsed.sessionId, '019e0000-0000-7000-8000-000000000003');
  assert.equal(structured.answer.includes('fixture structured answer'), true);
...
test('parseCodexOutput does not promote commentary or progress response_item text to result content', () => {
  const stdout = [
    JSON.stringify({ type: 'thread.started', thread_id: '019e0000-0000-7000-8000-000000000002' }),
