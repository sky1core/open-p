...
appendLog({ type: 'turn_context', payload: {
  model: 'codex-test-model',
  // Opt-in so the turn can report an effort and a sandbox policy that differ from what was asked
  // for, which is the only way to tell a reported value from an echoed request.
  ...(process.env.OPENP_FAKE_CODEX_REPORTED_EFFORT
    ? { effort: process.env.OPENP_FAKE_CODEX_REPORTED_EFFORT }
    : {}),
  ...(process.env.OPENP_FAKE_CODEX_REPORTED_SANDBOX
    ? { sandbox_policy: { type: process.env.OPENP_FAKE_CODEX_REPORTED_SANDBOX } }
    : {}),
} });
appendLog({
  type: 'response_item',
