...
test('formats backend-reported effort in json and stream-json result metadata', () => {
  for (const outputFormat of ['json', 'stream-json'] as const) {
    const [openp] = parseOpenPRecords(formatTurnResult({
      ...RESULT,
      diagnostics: {
        ...RESULT.diagnostics,
    ...
    assert.ok(openp);
    assert.equal(metadata(openp).actualEffort, 'actual-effort-alpha');
  }
});

test('does not fill result metadata effort from a caller-requested effort', () => {
  const outputOptions = {
    outputFormat: 'json',
    backendSessionId: '11111111-1111-4111-8111-111111111111',
    backend: 'claude',
    model: 'requested-model-beta',
    requestedEffort: 'requested-effort-must-not-leak',
    effort: 'requested-effort-must-not-leak',
  } as const;
  const openp = parseJsonOpenP(formatTurnResult({
    ...RESULT,
    diagnostics: {
  ...
  assert.equal(metadata(openp).actualEffort, null);
  assert.equal(metadata(openp).requestedEffort, 'requested-effort-must-not-leak');
});

test('reports a silent effort substitution as a difference between requested and actual', () => {
  // A backend may accept an effort it does not recognize and run its own instead without saying so.
  // Both values ride on the result so the caller can see that happened without holding on to what it
  // asked for.
  for (const outputFormat of ['json', 'stream-json'] as const) {
    const [openp] = parseOpenPRecords(formatTurnResult({
      ...RESULT,
      ...
      backend: 'claude',
      model: 'requested-model-alpha',
      requestedEffort: 'requested-effort-alpha',
    }));

    assert.ok(openp);
    assert.equal(metadata(openp).requestedEffort, 'requested-effort-alpha');
    assert.equal(metadata(openp).actualEffort, 'actual-effort-alpha');
  }
});

test('reports a null requested effort when the caller asked for none', () => {
  const openp = parseJsonOpenP(formatTurnResult({
    ...RESULT,
    diagnostics: {
      ...RESULT.diagnostics,
  ...
  assert.equal(metadata(openp).requestedEffort, null);
  assert.equal(metadata(openp).actualEffort, 'actual-effort-alpha');
});
...
test('formats worker result with full diagnostics in openp metadata', () => {
  ...
  assert.equal(metadata(openp).actualEffort, 'worker-actual-effort');
});

test('does not fill worker result metadata effort from caller event data', () => {
  const openp = parseSingleOpenPRecord(formatWorkerTurnResult({
    ...
    turnId: 'public-turn-requested-effort',
    model: 'claude-test',
    requestedEffort: 'requested-effort-must-not-leak',
    effort: 'requested-effort-must-not-leak',
  } as Parameters<typeof formatWorkerTurnResult>[1] & { readonly effort: string }));

  assert.equal(metadata(openp).actualEffort, null);
  assert.equal(metadata(openp).requestedEffort, 'requested-effort-must-not-leak');
});

test('does not fill worker result metadata requested effort from the backend-reported one', () => {
  const openp = parseSingleOpenPRecord(formatWorkerTurnResult(WORKER_RESULT, {
    turnId: 'public-turn-unrequested-effort',
    model: 'claude-test',
  }));
...
test('does not fill result metadata permission mode from a caller-requested one', () => {
  const outputOptions = {
    outputFormat: 'json',
    backendSessionId: '11111111-1111-4111-8111-111111111111',
    backend: 'claude',
    requestedPermissionMode: 'requested-mode-must-not-leak',
  } as const;
  const openp = parseJsonOpenP(formatTurnResult({
    ...RESULT,
    diagnostics: { ...RESULT.diagnostics, permissionMode: undefined },
  }, outputOptions));

  assert.equal(metadata(openp).actualPermissionMode, null);
  assert.equal(metadata(openp).requestedPermissionMode, 'requested-mode-must-not-leak');
});

test('does not fill result metadata requested permission mode from the backend-reported one', () => {
  const openp = parseJsonOpenP(formatTurnResult({
    ...RESULT,
    diagnostics: { ...RESULT.diagnostics, permissionMode: 'actual-mode-alpha' },
  }, {
    outputFormat: 'json',
    backendSessionId: '11111111-1111-4111-8111-111111111111',
    backend: 'claude',
  ...
  assert.equal(metadata(openp).actualPermissionMode, 'actual-mode-alpha');
  assert.equal(metadata(openp).requestedPermissionMode, null);
});

test('reports a permission mode the backend settled on as a difference from the requested one', () => {
  // A policy or a managed requirements file can put a turn under a different mode without failing
  // it. Both values ride on the result so the caller can see that happened.
  for (const outputFormat of ['json', 'stream-json'] as const) {
    const [openp] = parseOpenPRecords(formatTurnResult({
      ...RESULT,
      diagnostics: { ...RESULT.diagnostics, permissionMode: 'actual-mode-alpha' },
      ...
      backendSessionId: '11111111-1111-4111-8111-111111111111',
      backend: 'claude',
      requestedPermissionMode: 'requested-mode-alpha',
    }));

    assert.ok(openp);
    assert.equal(metadata(openp).requestedPermissionMode, 'requested-mode-alpha');
    assert.equal(metadata(openp).actualPermissionMode, 'actual-mode-alpha');
  }
});

test('does not fill worker result metadata permission mode from caller event data', () => {
  const openp = parseSingleOpenPRecord(formatWorkerTurnResult({
    ...WORKER_RESULT,
    diagnostics: { ...WORKER_RESULT.diagnostics, permissionMode: undefined },
  }, {
    turnId: 'public-turn-requested-mode',
    requestedPermissionMode: 'requested-mode-must-not-leak',
  }));
...
test('does not fill worker result metadata requested permission mode from the backend-reported one', () => {
  const openp = parseSingleOpenPRecord(formatWorkerTurnResult({
    ...WORKER_RESULT,
    diagnostics: { ...WORKER_RESULT.diagnostics, permissionMode: 'actual-mode-alpha' },
  }, { turnId: 'public-turn-unrequested-mode' }));
