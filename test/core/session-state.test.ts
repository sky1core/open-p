...
test('pending seed session marker is strict, private, and rejected by ordinary v1 state loading', async () => {
  ...
  for (const forbidden of ['codename FIXTURESECRET', 'noted-secret', 'raw-external-id', 'userText', 'assistantText']) {
...
function sessionStateTestJournal(projectRoot: string) {
  ...
  const written: readonly NativeWrittenTurn[] = [{
    ...
    contentDigest: contentDigest('codename FIXTURESECRET', 'noted-secret'),
    nativeIds: sessionStateIds('seeded'),
  }];
  const fullTurns = [
    ...beforeTurns,
    { userText: 'codename FIXTURESECRET', assistantText: 'noted-secret', nativeIds: sessionStateIds('seeded') },
  ];
