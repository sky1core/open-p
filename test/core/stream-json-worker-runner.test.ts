...
test('stream-json worker reports the requested reasoning effort on the result it emits', async () => {
  // The requested effort only earns its place on the result by arriving there; a caller comparing it
  // against the backend-reported one learns nothing from a field that is always null. This bridge
  // reports no effort of its own, so the requested value must also not stand in for it.
  for (const streaming of [false, true]) {
    ...
    await runStreamJsonWorkerLines({
      options: options({ reasoningEffort: 'high', streaming }),
      lines: lines([userEvent('turn-a', 'prompt')]),
      bridge,
      ...state,
      outputMetadata: metadata(),
    ...
    const results = output
      ...
      .split('\n')
      .filter(Boolean)
      .map((line) => JSON.parse(line) as { openp?: { form?: string; metadata?: Record<string, unknown> } })
      .filter((record) => record.openp?.form === 'result');

    assert.equal(results.length, 1, `streaming=${streaming} emitted ${results.length} result records`);
    assert.equal(results[0]!.openp!.metadata!.requestedEffort, 'high');
    assert.equal(results[0]!.openp!.metadata!.actualEffort, null);
  }
});
