...
if (args[0] === 'display-message' && args.includes('#{cursor_y}:#{cursor_x}')) {
  process.stdout.write('3:2\\n');
  process.exit(0);
}
if (args[0] === 'capture-pane') {
...
`);
  ...
  assert.deepEqual(await session.captureCursorSurface(), {
    line: '❯',
    cursorRow: 3,
    cursorColumn: 2,
  });
  ...
  assert.deepEqual(commandLines[2], ['display-message', '-p', '-t', 'fake-session', '#{cursor_y}:#{cursor_x}']);
  assert.deepEqual(commandLines[3], ['capture-pane', '-p', '-t', 'fake-session', '-S', '3', '-E', '3']);
  assert.deepEqual(commandLines[4], ['display-message', '-p', '-t', 'fake-session', '#{cursor_y}:#{cursor_x}']);
  assert.deepEqual(commandLines[5], ['capture-pane', '-p', '-t', 'fake-session', '-S', '3', '-E', '3']);
  assert.deepEqual(commandLines[6], ['display-message', '-p', '-t', 'fake-session', '#{cursor_y}:#{cursor_x}']);
});

test('tmux cursor-surface capture rejects same-column row movement', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'openp-fake-tmux-'));
  const fakeTmux = join(dir, 'fake-tmux.js');
  const logPath = join(dir, 'commands.log');
  await writeFile(fakeTmux, `#!/usr/bin/env node
...
fs.appendFileSync(logPath, JSON.stringify(args) + '\\n');
if (args[0] === 'display-message' && args.includes('#{cursor_y}:#{cursor_x}')) {
  const positionReads = previous.filter((entry) => entry.includes('#{cursor_y}:#{cursor_x}')).length;
  process.stdout.write(positionReads === 0 ? '3:2\\n' : '4:2\\n');
  process.exit(0);
}
if (args[0] === 'capture-pane') {
  process.stdout.write('❯\\n');
...
`);
  ...
  await assert.rejects(
    session.captureCursorSurface(),
    /cursor moved while its input surface was captured/,
  );
});
