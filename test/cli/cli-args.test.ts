...
test('accepts a native permission mode as a public option', () => {
  const options = parseCliArgs(['claude', '--permission-mode', 'fixture-mode-alpha', 'hello'], KNOWN_BACKENDS);
  assert.equal(options.nativePermissionMode, 'fixture-mode-alpha');
  assert.equal(options.permissionMode, null);
});

test('rejects giving both permission options because they set the same thing', () => {
  // Neither wins: open-p does not decide which one the caller meant.
  for (const argv of [
    ['claude', '--dangerously-skip-permissions', '--permission-mode', 'fixture-mode-alpha', 'hello'],
    ['claude', '--permission-mode', 'fixture-mode-alpha', '--dangerously-skip-permissions', 'hello'],
  ]) {
    assert.throws(
      () => parseCliArgs(argv, KNOWN_BACKENDS),
      (error: unknown) => error instanceof OpenPError && error.exitCode === 2,
      `expected a usage error for ${argv.join(' ')}`,
    );
  }
});

test('keeps the trusted-tool intent apart from a native permission mode', () => {
  const options = parseCliArgs(['claude', '--dangerously-skip-permissions', 'hello'], KNOWN_BACKENDS);
  assert.equal(options.permissionMode, 'danger-full-access');
  assert.equal(options.nativePermissionMode, null);
});
